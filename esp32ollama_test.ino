// This code works on generic ESP32-WROOM-32E module on Arduino IDE 2.3.2 Tools --- > ESP32 ---> ESP32 Dev Module
...
// The code goes to deep sleep after all prompts sending and receiving
// Version 1
// Partha Pratim Ray
// 12 September, 2024 
...
void sendPromptToOllama(String prompt) {
  ...
  if (http.begin(client, ollama_api_url)) {
    ...
    http.setTimeout(30000);  // Increase timeout to 30 seconds to allow for slower response from Raspberry Pi
    ...
    if (httpResponseCode > 0) {
      ...
      // Process the nested JSON string
      JSONVar nestedJson = JSON.parse(nestedJsonString);
      if (JSON.typeof(nestedJson) == "undefined") {
        Serial.println("Parsing nested JSON failed!");
        return;
      }
